...
$('#kod').hide(50);
    $("a.dede").click(function(){
        i++;
        $("div#kur").append("<input type=\"file\" name=\"resim[]\" id=\"resim-"+i+"\" lang=\"tr\"><a data-id=\""+i+"\" class=\"kaldir btn green\">Kaldır</a>");

    });
    $('a.kaldir').live('click',function(){
        var button = $(this);
    ...
    $("a.hizm").click(function(){
        i++;
        $("div#hi").append("<input type=\"text\" name=\"hizmet[]\" id=\"hizmet-"+i+"\" lang=\"tr\"><a data-id=\""+i+"\" class=\"hizkaldir btn green\">Kaldır</a>");

    });
    $('a.hizkaldir').live('click',function(){
        var button = $(this);
        var id = button.attr('data-id');
        $('#hizmet-'+id).remove();
        button.remove();
	...
	$("#reklam input[type='radio']").change(function(){
		var xasd = $(this).val();
		...
		if(xasd=="gogil"){
			...
			$('#res').hide(500);
			$('#kod').show(500);
			$('input[name="hangi"]').val("gogil");
		}else if(xasd="normal"){
			$('#ling').show(50);
			$('#res').show(500);
			$('#kod').hide(500);
			$('input[name="hangi"]').val("normal");
		}
	
	});
	function resimkaldir(){
        $('#kalk').hide(500).empty();
        $('#kalk').fadeIn(500).html('<section><label for="file_upload">Resim</label><div><input type="file" name="resim" id="resim" lang="tr"/></div></section>');
    }
    ...
    $content.find('table.chart').wl_Chart({
        onClick:function(value, legend, label, id){
            $.msg("value is "+value+" from "+legend+" at "+label+" ("+id+")",{
...
/*-----------------------------------------------------------------------------------------------------------------------------*/
    ...
    $content.find('#save_store').bind('click',function(){
        $.prompt('Storing some values?','This text is for the storage',function(value){
            ...
            if(wl_Store.save('testvalue', value)){
                $.msg('Your data has been saved!. You can reload the page now!',{
                    live:10000
                });
            }else{
    ...
    $('#message_methods').click(function(){
        var m = $.msg("This message can be accessed via public methods",{
            sticky:true
        });
